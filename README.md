...
Drawing tools use **I** (Eyedropper), **B** (Brush), **Shift E** (Eraser), **F** (Fill), **P** (Pen) and **G** (Gradient); **E** still creates an ellipse. The contextual options bar contains the active tool’s controls; raster tools and Eyedropper share foreground/background colours. Eyedropper samples the visible composite, including effects and background; transparent samples leave the current colour unchanged.
