import { clone, validateDocument, migrateDocument } from '../model/document.js';
...
export async function saveProject(document, assets) {
  ...
  const blobs = new Map([...used].map(id => [id, assets.blobs.get(id)]));
  const db = await openDatabase();
  try {
    const tx = db.transaction(['projects', 'assets'], 'readwrite'), done = completed(tx);
    try {
      tx.objectStore('projects').put(doc);
      for (const id of used) tx.objectStore('assets').put({ id, blob: blobs.get(id) });
...
export async function loadProject(id) {
  const db = await openDatabase(), doc = migrateDocument(await result(db.transaction('projects').objectStore('projects').get(id)));
